...
class vonline_Theme_Dashboard {
	 ...
	 */
	public $starter_plugin_slug = 'vonline-starter-sites';
	 ...
	 */
	public $starter_plugin_path = 'vonline-starter-sites/vonline-starter-sites.php';
	...
	public function add_menu_page() {
		add_submenu_page( 'themes.php', esc_html__( 'Theme Dashboard', 'vonline' ), esc_html__( 'Theme Dashboard', 'vonline' ), 'manage_options', $this->menu_slug, array( $this, 'html_carcase' ), 1 );
	}
	...
	public function admin_enqueue_scripts( $page ) {
		...
		wp_enqueue_script( 'vonline', get_template_directory_uri() . '/theme-dashboard/scripts.js', array( 'jquery' ), filemtime( get_template_directory() . '/theme-dashboard/scripts.js' ), true );

		wp_localize_script( 'vonline', 'thd_localize', array(
			'ajax_url'       => admin_url( 'admin-ajax.php' ),
			'nonce'          => wp_create_nonce( 'nonce' ),
			'failed_message' => esc_html__( 'Something went wrong, contact support.', 'vonline' ),
		) );

		// Styles.
		wp_enqueue_style( 'vonline', get_template_directory_uri() . '/theme-dashboard/style.css', array(), filemtime( get_template_directory() . '/theme-dashboard/style.css' ) );

		// Add RTL support.
		wp_style_add_data( 'vonline', 'rtl', 'replace' );
	}
	...
	public function install_starter_plugin() {
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error( esc_html__( 'Insufficient permissions to install the plugin.', 'vonline' ) );
			wp_die();
		}
		...
		wp_send_json_error( esc_html__( 'Failed to initialize or activate importer plugin.', 'vonline' ) );
					...
					<div class="thd-theme-feature-row">
						...
						if ( isset( $feature['customize_uri'] ) && $feature['customize_uri'] ) {
							...
							<a href="<?php echo esc_attr( $customize_uri ); ?>" class="thd-theme-feature-customize" target="_blank">
								<?php esc_html_e( 'Customize', 'vonline' ); ?>
							</a>
						<?php } ?>
					</div>
				</div>
		...
		<div class="thd-performance">
				...
				<div class="thd-performance-item-outer">
					...
					<div class="thd-performance-item-content">
						<div class="thd-performance-item-name">
							<?php esc_html_e( 'Best WordPress Speed Optimization Plugins', 'vonline' ); ?>
						</div>

						<div class="thd-performance-item-desc">
							<?php esc_html_e( 'We run through the top 10 WordPress performance plugins to keep your site running fast. Goes beyond just caching plugins (though those are on there too!). A must-read for all WordPress site owners.', 'vonline' ); ?>
						</div>

						<a class="thd-performance-item-read-more" href="https://vanstoneonline.com/collections/best-wordpress-speed-optimization-plugins/" target="_blank">
							<?php esc_html_e( 'Read more', 'vonline' ); ?>
						</a>
					</div>
				</div>
			</div>
			<div class="thd-performance-item">
				<div class="thd-performance-item-outer">
					...
					<div class="thd-performance-item-content">
						<div class="thd-performance-item-name">
							<?php esc_html_e( 'WP Rocket Review', 'vonline' ); ?>
						</div>

						<div class="thd-performance-item-desc">
							<?php esc_html_e( 'Every site that is serious about speed (and should be pretty much all sites these days) needs a caching plugin and WP Rocket is our pick of the bunch. Get the lowdown on why we recommend it n our data-backed review: ', 'vonline' ); ?>
						</div>

						<a class="thd-performance-item-read-more" href="https://vanstoneonline.com/reviews/wp-rocket-review/" target="_blank">
							<?php esc_html_e( 'Read more', 'vonline' ); ?>
						</a>
					</div>
				</div>
			...
			<div class="thd-performance-item">
				<div class="thd-performance-item-outer">
					...
					<div class="thd-performance-item-content">
						<div class="thd-performance-item-name">
							<?php esc_html_e( 'Fastest WordPress Hosts', 'vonline' ); ?>
						</div>

						<div class="thd-performance-item-desc">
							<?php esc_html_e( 'A slow host can really put a drag on your site’s performance, even an optimized one. Make sure you get a host that won’t hold you back by reading our review, complete with real speed tests, of the fastest WordPress hosts. ', 'vonline' ); ?>
						</div>

						<a class="thd-performance-item-read-more" href="https://vanstoneonline.com/reviews/fastest-wordpress-hosting/" target="_blank">
							<?php esc_html_e( 'Read more', 'vonline' ); ?>
						</a>
					</div>
				</div>
			...
			<div class="thd-hero-content">
				...
				<div class="thd-hero-actions">
					...
					<a href="<?php echo esc_url( add_query_arg( 'page', $this->starter_menu_slug, admin_url( 'themes.php' ) ) ); ?>" data-target="<?php echo esc_attr( $target ); ?>" class="thd-hero-go button button-primary">
						<?php esc_html_e( 'Starter Sites', 'vonline' ); ?>
					</a>
					...
					<?php if ( 'themes.php' === $pagenow && 'themes' === $screen->base ) { ?>
						<a href="<?php echo esc_url( add_query_arg( 'page', $this->menu_slug, admin_url( 'themes.php' ) ) ); ?>" class="button">
							<?php esc_html_e( 'Theme Dashboard', 'vonline' ); ?>
						</a>
					<?php } ?>
				</div>
				...
				<?php if ( 'active' !== $this->get_plugin_status( $this->starter_plugin_path ) ) { ?>
					<div class="thd-hero-notion">
						<?php esc_html_e( 'Clicking “Starter Sites” button will install and activate the demo importer plugin.', 'vonline' ); ?>
					</div>
				<?php } ?>
			</div>
	...
	public function html_carcase() {
		...
		<div class="thd-wrap thd-theme-dashboard">
			<div class="thd-header">
				<div class="thd-header-left">
					<div class="thd-header-col thd-header-col-logo">
						<div class="thd-logo">
							<a target="_blank" href="<?php echo esc_url( 'https://vanstoneonline.com/' ); ?>">
								<img width="96px" height="24px" src="<?php echo esc_url( get_template_directory_uri() . '/theme-dashboard/images/logo.svg' ); ?>" alt="<?php esc_attr_e( 'vonline', 'vonline' ); ?>">
							</a>
						</div>
					</div>
				</div>
				...
				<div class="thd-header-right">
					<a href="<?php echo esc_url( $this->settings['documentation_link'] ); ?>" class="thd-link-documentation" target="_blank">
						<span><?php esc_html_e( 'Documentation', 'vonline' ); ?></span>
			...
			<div class="wrap">
				...
				<h1 class="hidden"><?php esc_html_e( 'Theme Dashboard', 'vonline' ); ?></h1>
				...
				<div class="thd-main">
					<div class="thd-main-content">
						...
						<div class="thd-panel thd-panel-support">
							<div class="thd-panel-head">
								<h3 class="thd-panel-title"><?php esc_html_e( 'Support', 'vonline' ); ?></h3>

								<?php if ( $this->pro_status ) { ?>
									<div class="thd-badge thd-badge-success"><?php esc_html_e( 'Priority Status', 'vonline' ); ?></div>
								<?php } ?>
							</div>
							<div class="thd-panel-content">
								<div class="thd-conttent-primary">
									<div class="thd-title">
										<?php echo wp_kses_post( __( 'Need help? We\'re here for you!', 'vonline' ) ); ?>
									</div>

									<div class="thd-description"><?php esc_html_e( 'Have a question? Hit a bug? Get the help you need, when you need it from our friendly support staff.', 'vonline' ); ?></div>

									<div class="thd-button-wrap">
										<a href="<?php echo esc_url( $this->settings['support_link'] ); ?>" class="thd-button button" target="_blank">
											<?php echo esc_html_e( 'Get Support', 'vonline' ); ?>
										</a>
									</div>
								</div>
								...
								<?php if ( ! $this->pro_status ) { ?>
									<div class="thd-conttent-secondary">
										<div class="thd-title">
											<?php echo wp_kses_post( __( 'Priority Support', 'vonline' ) ); ?>

											<div class="thd-badge"><?php esc_html_e( 'pro', 'vonline' ); ?></div>
										</div>

										<div class="thd-description"><?php esc_html_e( 'Want your questions answered faster? Go Pro to be first in the queue!', 'vonline' ); ?></div>

										<div class="thd-button-wrap">
											<a href="<?php echo esc_url( $this->settings['support_pro_link'] ); ?>" class="thd-button button" target="_blank">
												<?php echo esc_html_e( 'Go PRO', 'vonline' ); ?>
											</a>
										</div>
									</div>
								<?php } ?>
						...
						<div class="thd-panel thd-panel-community">
							<div class="thd-panel-head">
								<h3 class="thd-panel-title"><?php esc_html_e( 'Community', 'vonline' ); ?></h3>
							</div>
							<div class="thd-panel-content">
								<div class="thd-title">
									<?php echo wp_kses_post( __( 'Join our Facebook community', 'vonline' ) ); ?>
								</div>

								<div class="thd-description"><?php esc_html_e( 'Discuss products and ask for community support or help the community.', 'vonline' ); ?></div>

								<div class="thd-button-wrap">
									<a href="<?php echo esc_url( $this->settings['community_link'] ); ?>" class="thd-button button" target="_blank">
										<?php echo esc_html_e( 'Join Now', 'vonline' ); ?>
									</a>
								</div>
							</div>
						</div>
					...
					<div class="thd-main-sidebar">
						...
						<div class="thd-panel thd-panel-review">
							<div class="thd-panel-head">
								<h3 class="thd-panel-title"><?php esc_html_e( 'Review', 'vonline' ); ?></h3>
							</div>
							<div class="thd-panel-content">
								...
								<div class="thd-description"><?php esc_html_e( 'It makes us happy to hear from our users. We would appreciate a review.', 'vonline' ); ?></div>

								<div class="thd-button-wrap">
									<a href="<?php echo esc_url( $this->settings['review_link'] ); ?>" class="thd-button button" target="_blank">
										<?php echo esc_html_e( 'Submit a Review', 'vonline' ); ?>
									</a>
								</div>

								<div class="thd-line"></div>

								<div class="thd-heading"><?php esc_html_e( 'Have an idea how to improve?', 'vonline' ); ?></div>

								<div class="thd-description"><?php esc_html_e( 'Let us know. We process all suggestions to make the theme perfect.', 'vonline' ); ?></div>

								<a href="<?php echo esc_url( $this->settings['suggest_idea_link'] ); ?>" class="thd-suggest-idea-link" target="_blank">
									<?php echo esc_html_e( 'Suggest an Idea', 'vonline' ); ?>
								</a>
							</div>
						</div>
						...
						<div class="thd-panel thd-panel-changelog">
							<div class="thd-panel-head">
								<h3 class="thd-panel-title"><?php esc_html_e( 'Changelog', 'vonline' ); ?></h3>
							...
							<div class="thd-panel-content">
								<div class="thd-description"><?php esc_html_e( 'Keep informed with the latest changes about each theme.', 'vonline' ); ?></div>

								<a href="<?php echo esc_url( $this->settings['changelog_link'] ); ?>" class="thd-changelog-link" target="_blank">
									<?php echo esc_html_e( 'See the Changelog', 'vonline' ); ?>
								</a>
							</div>
						</div>
					</div>
