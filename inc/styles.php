...
function vonline_hex2rgba($color, $opacity = false) {
...
//Dynamic styles
function vonline_custom_styles($custom) {
	...
	if ( class_exists( 'Woocommerce' ) && is_shop() && $shop_thumb ) {
		...
		$custom .= "@media only screen and (max-width: 1024px) { .vonline-hero-area .header-image { height:300px!important; }}" . "\n";
		...
		$rgba 	= vonline_hex2rgba($menu_bg_color, 0.9);
	...
	//Primary color SVGs
	$custom .= ".go-top:hover svg,.vonline_contact_info_widget span { fill:" . esc_attr( $primary_color ) . ";}" . "\n";
	//Menu background
	...
	$custom .= ".btn-menu .vonline-svg-icon { fill:" . esc_attr($mobile_menu_color) . "}"."\n";
	...
	$custom = apply_filters( 'vonline_custom_css', $custom );
	
	//Output all the styles
	wp_add_inline_style( 'vonline-style', $custom );	
}
add_action( 'wp_enqueue_scripts', 'vonline_custom_styles' );